...
//		System.out.println(c);
	...
	public List<AssociationRule> assRule(ItemSetCollection db,
			ItemSetCollection l, double thordConfidence) {
		...
		for (int i = 0; i < l.size(); i++) {
			if (l.get(i).size() > 1) {
				...
				it.addAll(subItemSet(l.get(i)));// gia tri tap cac itemset con  tu 1 itemset trong tap PB l
				double xy = support(l.get(i), db);
				for (int j = 0; j <it.size(); j++) {
					double x=support(it.get(j), db);
					double confidence = (double)(xy/x)*100;
					ItemSet its = new ItemSet();
					its.addAll(l.get(i));
					if(confidence>=thordConfidence){
